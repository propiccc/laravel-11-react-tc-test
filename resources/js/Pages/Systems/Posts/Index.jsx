...
const Index = () => {
    ...
    const getDataTable = (url = null) => {
        var url = url == null ? '/api/v1/posts?' + `limit=${Paginate.limit}&search=${Paginate.search}` : '/api' + url.split('/api')[1] + `&limit=${Paginate.limit}&search=${Paginate.search}`;
        setBlock(true);
    ...
    const getDataEdit = async (uuid) => {
        ...
        try {
            const res = await axios.post(`/api/v1/posts/${uuid}/show`);
    ...
    const DeleteData = (uuid) => {
        ...
        setFormToggle(false);
        setDataEdit(null);
        var url = `/api/v1/posts/${uuid}/delete`
    ...
    // FIXME : Update Type Jika Update
    // FIXME : Error Handling GetDataEdit Function
                                ...
                                <table
                                    ...
                                    <thead
                                        ...
                                        <tr className='sticky top-0'>
                                            ...
                                            <th scope="col" className="whitespace-nowrap py-2 text-start">Title</th>
                                            <th scope="col" className="whitespace-nowrap py-2 px-5 text-start">Content</th>
                                            <th scope="col" className="whitespace-nowrap py-2 text-start w-[100px]">Created At</th>
                                            ...
                                            Data?.data?.map((item, index) => (
                                                <>
                                                    <tr key={index} className="border-b border-gray-200">
                                                        ...
                                                        <td className="whitespace-pre-line py-2 ">{item.title}</td>
                                                        <td className="whitespace-pre-line py-2 px-5 w-[400px]">{item.content}</td>
                                                        <td className="whitespace-nowrap py-2 text-center">{moment(`${item.created_at}`).format('MMMM DD YYYY, h:mm:ss')}</td>
